...
void main()
{
    ...
    if(cnoise(grid)>0.5-blurAmnt*.3){
        gl_FragColor=texture2DRect(tex0, texCoordVarying+vec2(blurAmnt*100*cnoise(grid),0));
        ...
        if(cnoise(grid2)>.8){
            //gl_FragColor=texture2DRect(tex0, texCoordVarying+vec2(-blurAmnt,0));
            float r=cnoise(grid2)/2.0;
